...
func init() {
	...
	flag.IntVar(&flagMaxJobs, "max-jobs", 0, "maximum distinct jobs to process before stopping (0 means unlimited)")
}
...
func main() {
	...
	claimedJobs := make(map[int64]struct{})
	for {
		if shutdownCtx.Err() != nil {
			logger.Info("stopped before claiming another HQ job")
			return
		}
		if flagMaxJobs > 0 && len(claimedJobs) >= flagMaxJobs {
			logger.Info("reached job limit", zap.Int("claimed_jobs", len(claimedJobs)))
			return
		}
