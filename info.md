...
- **存档工具**：`cmd/archive/` — 从 SavewebHQ 的 `sinavideo` 项目领取 job（`value` 为 vid），自动查询 API、探测源站、下载视频和元数据，全部写入 WARC 后经 tus 上传。HQ 地址使用 SDK 默认的 `https://hq.saveweb.org/`；运行时只需设置 `HQ_MACHINE_TOKEN`，贡献者身份由 machine token 自动解析。
